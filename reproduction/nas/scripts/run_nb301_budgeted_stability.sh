...
OUT_ROOT="${OUT_ROOT:-${NAS_PACKAGE_DIR}/outputs/nb301_budgeted_stability}"
OP_SCORE_ROOT="${OP_SCORE_ROOT:-${NAS_PACKAGE_DIR}/outputs/nb301_main/operation_scores}"
REFINEMENT_PY="${REFINEMENT_PY:-${PY:-python}}"
REFINEMENT_LD_LIBRARY_PATH="${REFINEMENT_LD_LIBRARY_PATH:-}"
REPORTED_DIR="$(cd "${NAS_PACKAGE_DIR}/../nas_reported" && pwd)"
FULL_SELECTION_DETAILS="${FULL_SELECTION_DETAILS:-${NAS_PACKAGE_DIR}/outputs/nb301_main/caches/full_proxy_pool_details.json}"
SUBSET_MANIFEST="${SUBSET_MANIFEST:-${OUT_ROOT}/nb301_subset_manifest.json}"
...
run_subset_refinement() {
  ...
  bash "$SCRIPT_DIR/run_nb301_refinement.sh" \
...
SUBSET_TOTAL_STEPS="${SUBSET_TOTAL_STEPS:-60}"
SUBSET_RESIDUAL_SCALE="${SUBSET_RESIDUAL_SCALE:-1.00}"
...
"$REFINEMENT_PY" "$REPORTED_DIR/generate_nb301_subset_manifest.py" \
...
echo "===== ProxyDiff NB301 budgeted stability START $(date '+%F %T') =====" | tee "$MASTER_LOG"
...
echo "subset_manifest=$SUBSET_MANIFEST" | tee -a "$MASTER_LOG"
echo "subset_pool_filter=$SUBSET_POOL_FILTER" | tee -a "$MASTER_LOG"
...
if (( SUBSET_SHARD_COUNT == 1 )); then
  run_with_optional_ld "$REFINEMENT_LD_LIBRARY_PATH" "$REFINEMENT_PY" "$NAS_SRC_DIR/summarize_nb301_budgeted_results.py" \
    --out_root "$OUT_ROOT/refine" \
    --out_csv "$OUT_ROOT/nb301_budgeted_stability_summary.csv" \
    | tee "$OUT_ROOT/nb301_budgeted_stability_summary_stdout.csv"
fi

echo "===== ProxyDiff NB301 budgeted stability DONE $(date '+%F %T') =====" | tee -a "$MASTER_LOG"
