...
SEED="${SEED:-9000}"
CACHE_DETAILS="${CACHE_DETAILS:-${CACHE%_proxydiff_cache.pt}_details.json}"

if [[ -z "$REFINEMENT_LD_LIBRARY_PATH" ]]; then
  REFINEMENT_ENV_LIB="$(cd "$(dirname "$REFINEMENT_PY")/../lib" 2>/dev/null && pwd -P || true)"
  if [[ -n "$REFINEMENT_ENV_LIB" ]] && compgen -G "$REFINEMENT_ENV_LIB/libcusparse.so*" >/dev/null; then
    REFINEMENT_LD_LIBRARY_PATH="$REFINEMENT_ENV_LIB"
  fi
fi
...
if [[ ! -f "$CACHE_DETAILS" ]]; then
  echo "ProxyDiff cache details not found: $CACHE_DETAILS" >&2
  exit 2
fi
...
"$REFINEMENT_PY" "$NAS_PACKAGE_DIR/runtime/verify_bundled_runtime.py" \
  --runtime-root "$NAS_RUNTIME_ROOT"
"$REFINEMENT_PY" "$NAS_SRC_DIR/validate_nb301_refinement_cache.py" \
  --cache "$CACHE" \
...
if [[ "$PROXY_SET" == "full_proxy_pool" ]]; then
  ...
  AXIS_CALIBRATION_LR="${AXIS_CALIBRATION_LR:-0.015}"
  COMPONENT_CORRECTION_LR="${COMPONENT_CORRECTION_LR:-0.060}"
...
copy_refinement_artifacts() {
  ...
  cp "${latest}/proxydiff_epoch_-01_score_params.pt" "$artifact_dir/score_prior.pt"
  cp "${latest}/proxydiff_step_$(printf "%03d" "$axis_step")_mask_score_params.pt" "$artifact_dir/axis_calibrated_selection.pt"
  cp "${latest}/proxydiff_epoch_000_score_params.pt" "$artifact_dir/component_corrected_refinement.pt"
  cp "${latest}/proxydiff_epoch_-01_summary.json" "$artifact_dir/score_prior_summary.json"
  cp "${latest}/proxydiff_epoch_000_summary.json" "$artifact_dir/component_corrected_refinement_summary.json"
}

RUN_LABEL="${RUN_LABEL:-}"
if [[ -n "$RUN_LABEL" ]]; then
  if [[ ! "$RUN_LABEL" =~ ^[a-zA-Z0-9_]+$ ]]; then
    echo "RUN_LABEL may contain only letters, digits, and underscores" >&2
    exit 2
  fi
  RUN_NAME="proxydiff_nb301_refinement_${PROXY_SET}_${RUN_LABEL}"
else
  RUN_NAME="proxydiff_nb301_refinement_${PROXY_SET}"
fi
CONFIG_DIR="${NAS_RUNTIME_CONFIG_DIR:-configs/proxydiff_reproduction}"
CFG="${CONFIG_DIR}/${RUN_NAME}.yaml"

echo "===== ProxyDiff NB301 refinement START $(date '+%F %T') =====" | tee "$LOG_ROOT/master.log"
...
echo "axis_gradient_aggregation=${AXIS_GRADIENT_AGGREGATION:-temporal_unit_direction}" | tee -a "$LOG_ROOT/master.log"
echo "component_gradient_aggregation=${COMPONENT_GRADIENT_AGGREGATION:-stage_consensus_direction}" | tee -a "$LOG_ROOT/master.log"
...
export AXIS_CALIBRATION_LR="${AXIS_CALIBRATION_LR:-0.015}"
...
export PROXYDIFF_REFINEMENT_MOMENTUM=0
export PROXYDIFF_DIRECTION_EMA_DECAY=0.9
export PROXYDIFF_AXIS_GRADIENT_AGGREGATION="${AXIS_GRADIENT_AGGREGATION:-temporal_unit_direction}"
export PROXYDIFF_COMPONENT_GRADIENT_AGGREGATION="${COMPONENT_GRADIENT_AGGREGATION:-stage_consensus_direction}"
export PROXYDIFF_COMPONENT_CORR_LINK="unit_slope_bounded"
export PROXYDIFF_COMPONENT_CORR_CENTERING="raw"
export PROXYDIFF_REPLAY_STAGE_FEEDBACK_ORDER=0
export PROXYDIFF_TASK_FEEDBACK_SPLIT=train
export PROXYDIFF_RESET_FEEDBACK_AT_COMPONENT=1
...
export EVALUATE_DECODE_VARIANTS=0
...
echo "===== ProxyDiff NB301 refinement DONE $(date '+%F %T') latest=$LATEST =====" | tee -a "$LOG_ROOT/master.log"
