...
def summarize_artifact(path: Path, model, fixed_acc: np.ndarray, top_k: int):
    ...
    masks = obj.get("masks")
    mask_applied = masks is not None
    if mask_applied:
        if isinstance(masks, list):
            masks = torch.stack([torch.as_tensor(mask) for mask in masks], dim=0)
        mask_np = masks.detach().cpu().numpy().astype(bool)
        if mask_np.shape != score_np.shape:
            raise ValueError(
                f"progressive mask shape {mask_np.shape} does not match score shape {score_np.shape}: {path}"
            )
        if np.any(mask_np.sum(axis=2) < 2):
            raise ValueError(f"progressive mask leaves fewer than two legal operations on an edge: {path}")
        score_np = np.where(mask_np, score_np, -np.inf)
    ...
    return {
        ...
        "progressive_mask_applied": mask_applied,
        "decode": (
            "exact additive optimum within the saved progressive mask, with legal "
            "distinct-source pairs per NB301 node"
            if mask_applied
            else "exact additive optimum with legal distinct-source pairs per NB301 node"
        ),
    }
