...
def _axis_calibration_weights(search_space):
    if hasattr(search_space, "axis_calibration_weights"):
        return search_space.axis_calibration_weights
    return getattr(search_space, _runtime_weight_attr(1))


def _edge_epoch_weights(search_space):
    if hasattr(search_space, "edge_epoch_weights"):
        return search_space.edge_epoch_weights
    return getattr(search_space, _runtime_weight_attr(2))


def _component_corr_weights(search_space):
    if hasattr(search_space, "component_corr_weights"):
        return search_space.component_corr_weights
    return getattr(search_space, _runtime_weight_attr(3))
...
def _component_correction(raw):
    """Map unconstrained correction coordinates to bounded signed offsets."""
    link = os.environ.get(
        "PROXYDIFF_COMPONENT_CORR_LINK", "unit_slope_bounded"
    ).strip().lower()
    scale = float(os.environ.get("PROXYDIFF_COMPONENT_CORR_SCALE", "1.0") or 1.0)
    if link == "additive":
        correction = scale * raw
    elif link == "bounded":
        correction = scale * (torch.sigmoid(raw) - 0.5)
    elif link == "unit_slope_bounded":
        correction = scale * torch.tanh(raw)
    else:
        raise ValueError("unknown PROXYDIFF_COMPONENT_CORR_LINK={!r}".format(link))

    centering = os.environ.get(
        "PROXYDIFF_COMPONENT_CORR_CENTERING", "raw"
    ).strip().lower()
    if centering == "within_context_zero_mean":
        correction = correction - correction.mean(dim=-1, keepdim=True)
    elif centering != "raw":
        raise ValueError(
            "unknown PROXYDIFF_COMPONENT_CORR_CENTERING={!r}".format(centering)
        )
    return correction


def _mean_unit_gradient(gradients):
    """Average gradient directions while retaining their agreement magnitude."""
    if not gradients:
        raise ValueError("at least one gradient is required")
    unit_directions = []
    for gradient in gradients:
        vector = gradient.reshape(-1)
        norm = vector.norm().clamp_min(torch.finfo(vector.dtype).eps)
        unit_directions.append(vector / norm)
    return torch.stack(unit_directions, dim=0).mean(dim=0)


def _score_tensor_device(search_space):
    for metric_scores in getattr(search_space, "_score", []):
        for epoch_scores in metric_scores:
            for cell_score in epoch_scores:
                if torch.is_tensor(cell_score):
                    return cell_score.device
    for weights in (
        _axis_calibration_weights(search_space),
        _component_corr_weights(search_space),
    ):
...
def _build_nb301_proxydiff_cell_scores(search_space, epoch=0, include_component_corr=True):
    device = _score_tensor_device(search_space)
    score = [
        torch.zeros(search_space.num_edges, search_space.num_ops - 1, device=device)
        for _ in range(2)
    ]
    ...
    for cell in range(2):
        for m in range(search_space.metric_num):
            sum_m = torch.zeros(
                search_space.num_edges,
                search_space.num_ops - 1,
                device=device,
            )
        ...
        if include_component_corr and _has_component_corr(search_space):
            if search_space.metric_num != 0:
                if metric_weight_style == "latent_fixed_signed_residual":
                    score[cell] += _component_correction(
                        _component_corr_weights(search_space)[cell]
                    )
                else:
                    score[cell] += torch.sigmoid(
                        _component_corr_weights(search_space)[cell]
                    ) - 0.5
...
def _rank_equivalent_forward_scores(score):
    """Map signed search scores to a unit-mass mixed-operation bridge.

    NB301 architectures contain the same number of selected operations.  One
    global positive affine map therefore preserves every architecture-score
    ordering exactly. The mapped scores are scaled so the mean sum over the
    legal operations is one. Location and scale are detached, so task
    gradients remain a positive scalar multiple of the canonical gradients.
    """
    flat = torch.cat([cell.reshape(-1) for cell in score])
    lower = flat.detach().min()
    span = (flat.detach().max() - lower).clamp_min(torch.finfo(flat.dtype).eps)
    offset = torch.finfo(flat.dtype).eps
    mapped = [(cell - lower) / span + offset for cell in score]
    mapped_flat = torch.cat([cell.reshape(-1) for cell in mapped])
    operation_count = int(score[0].shape[-1])
    mass = (mapped_flat.detach().mean() * operation_count).clamp_min(
        torch.finfo(mapped_flat.dtype).eps
    )
    return [cell / mass for cell in mapped]


def install_nb301_canonical_score_forward(search_space):
    """Make NB301 training consume the same score tensor saved for decoding.

    The bundled NAS runtime constructs its own legacy metric mixture inside the
    root graph.  ProxyDiff keeps the runtime's network/data path, but replaces
    the metric tensor at each normal/reduction cell with the canonical builder
    above.  This keeps forward gradients and saved stage artifacts on one score
    equation without editing the external runtime checkout.
    """
    if getattr(search_space, "_proxydiff_canonical_forward_installed", False):
        return search_space

    original_root_forward = search_space.forward
    ...
    def root_forward(
        ...
        *args
        ...
        self._proxydiff_forward_epoch = int(epoch)
        canonical = _build_nb301_proxydiff_cell_scores(
            self,
            epoch=epoch,
            include_component_corr=_has_component_corr(self),
        )
        self._proxydiff_forward_scores = _rank_equivalent_forward_scores(canonical)
        try:
            return original_root_forward(
                x,
                drop_prob=drop_prob,
                epoch=epoch,
                to_print=to_print,
                fix_logger=fix_logger,
                no_edge_normalize=no_edge_normalize,
                *args
            )
        finally:
            self._proxydiff_forward_scores = None

    search_space.forward = types.MethodType(root_forward, search_space)

    seen = set()
    patched = []
    for node_index in search_space.nodes:
        node = search_space.nodes[node_index]
        cell_graph = node.get("subgraph") if hasattr(node, "get") else None
        cell_name = getattr(cell_graph, "name", None)
        if cell_graph is None or cell_name not in ("normal_cell", "reduction_cell"):
            continue
        if id(cell_graph) in seen:
            continue
        seen.add(id(cell_graph))
        cell_index = 0 if cell_name == "normal_cell" else 1
        original_cell_forward = cell_graph.forward
        ...
        def cell_forward(
            ...
            *args,
            ...
            _original_forward=original_cell_forward
        ):
            canonical = getattr(search_space, "_proxydiff_forward_scores", None)
            if canonical is None:
                epoch = int(getattr(search_space, "_proxydiff_forward_epoch", 0))
                canonical = _rank_equivalent_forward_scores(
                    _build_nb301_proxydiff_cell_scores(
                        search_space,
                        epoch=epoch,
                        include_component_corr=_has_component_corr(search_space),
                    )
                )
            return _original_forward(
                x,
                drop_prob=drop_prob,
                metric_para=canonical[_cell_index],
                mask=mask,
        ...
        cell_graph.forward = types.MethodType(cell_forward, cell_graph)
        patched.append(cell_name)

    if not patched:
        raise RuntimeError("NB301 canonical score adapter found no normal/reduction cells")
    search_space._proxydiff_canonical_forward_installed = True
    search_space._proxydiff_canonical_forward_cells = tuple(sorted(patched))
    return search_space
...
def _apply_nb301_topk_mask(search_space, score, topk):
    ...
    with torch.no_grad():
        for cell in range(2):
            ...
            score_rows = score[cell].detach().cpu().numpy()
            top_idx = np.stack(
                [
                    np.argsort(-row, kind="mergesort")[:keep_k]
...
class ZeroCostPredictorEvaluator(object):
    ...
    def single_evaluate(self, test_data, zc_api, perturbation=False, epoch=0, data=None):
                ...
                if self.search_space.has_metric_para:
                    ...
                    for cell in range(2):
                        ...
                        if self.search_space.metric_num != 0:
                            component_corr_raw = _component_corr_weights(self.search_space)[cell]
                            ...
                            score[cell] += _component_correction(component_corr_raw)
                            score2 = score[cell]
        ...
        if os.environ.get("PROXYDIFF_DUMP_EPOCH_ARTIFACTS", "1").strip().lower() in ("1", "true", "yes"):
            ...
            artifact = {
                ...
                "masks": _to_cpu_artifact(self.search_space._masks),
            ...
            if weight_path:
                ...
                if train_search_weights:
                    ...
                    if self.search_space.metric_num!=0:
                        ...
                        refinement_momentum = float(
                            os.environ.get("PROXYDIFF_REFINEMENT_MOMENTUM", "0") or 0.0
                        )
                        optimizer_axis_calib = torch.optim.SGD(
                            _axis_calibration_weights(self.search_space),
                            axis_calib_lr,
                            momentum=refinement_momentum,
                        ...
                        for metric_e in range(metric_epochs):
                            ...
                            for para_e in range(metric_epoch):
                                ...
                                task_feedback_split = os.environ.get(
                                    "PROXYDIFF_TASK_FEEDBACK_SPLIT",
                                    "train",
                                ).strip().lower()
                                if task_feedback_split == "train":
                                    task_feedback_loader = train_loader
                                elif task_feedback_split in ("validation", "valid"):
                                    task_feedback_loader = valid_loader
                                elif task_feedback_split == "axis_train_component_validation":
                                    axis_feedback_steps = int(
                                        os.environ.get(
                                            "PROXYDIFF_AXIS_CALIB_STEPS",
                                            "100",
                                        )
                                        or 100
                                    )

                                    def stage_capacity_feedback_loader():
                                        axis_batches = 0
                                        for batch in itertools.islice(
                                            ...
                                            axis_batches += 1
                                            yield batch
                                        if axis_batches != axis_feedback_steps:
                                            raise RuntimeError(
                                                "training loader yielded {} of {} requested axis batches".format(
                                                    axis_batches,
                                                    axis_feedback_steps,
                                                )
                                            )
                                    ...
                                    task_feedback_loader = stage_capacity_feedback_loader()
                                else:
                                    raise ValueError(
                                        "PROXYDIFF_TASK_FEEDBACK_SPLIT must be train, validation, "
                                        "or axis_train_component_validation"
                                    )
                                self.logger.info(
                                    "proxydiff_task_feedback_split: {}".format(
                                        task_feedback_split
                                    )
                                )
                                reset_feedback_at_component = os.environ.get(
                                    "PROXYDIFF_RESET_FEEDBACK_AT_COMPONENT",
                                    "1",
                                ).strip().lower() in ("1", "true", "yes")
                                if reset_feedback_at_component:
                                    feedback_source = task_feedback_loader
                                    axis_feedback_steps = int(
                                        os.environ.get("PROXYDIFF_AXIS_CALIB_STEPS", "30")
                                        or 30
                                    )
                                    total_feedback_steps = int(
                                        os.environ.get("PROXYDIFF_MAX_TRAIN_STEPS", "60")
                                        or 60
                                    )
                                    if total_feedback_steps <= axis_feedback_steps:
                                        raise ValueError(
                                            "total refinement steps must exceed axis steps"
                                        )

                                    def reset_stage_feedback_loader():
                                        for batch in itertools.islice(
                                            feedback_source, axis_feedback_steps
                                        ):
                                            yield batch
                                        replay_feedback_order = os.environ.get(
                                            "PROXYDIFF_REPLAY_STAGE_FEEDBACK_ORDER",
                                            "0",
                                        ).strip().lower() in ("1", "true", "yes")
                                        if replay_feedback_order:
                                            samplers = [
                                                getattr(feedback_source, "sampler", None),
                                                getattr(
                                                    getattr(feedback_source, "batch_sampler", None),
                                                    "sampler",
                                                    None,
                                                ),
                                            ]
                                            ...
                                            for sampler in samplers:
                                                if sampler is None or id(sampler) in seen_sampler_ids:
                                                    continue
                                                seen_sampler_ids.add(id(sampler))
                                                if hasattr(sampler, "_proxydiff_iter_count"):
                                                    sampler._proxydiff_iter_count = 0
                                                    reset_count += 1
                                            self.logger.info(
                                                "proxydiff_replay_stage_feedback_order: "
                                                "enabled=1 reset_samplers={}".format(reset_count)
                                            )
                                        for batch in itertools.islice(
                                            feedback_source,
                                            total_feedback_steps - axis_feedback_steps,
                                        ):
                                            yield batch

                                    task_feedback_loader = reset_stage_feedback_loader()
                                self.logger.info(
                                    "proxydiff_reset_feedback_at_component: {}".format(
                                        reset_feedback_at_component
                                    )
                                )
                                stop, decrease = self.train(
                                    task_feedback_loader,
                                    criterion,
                                    optimizer,
                                    optimizer_axis_calib,
                                    optimizer_metric2,
                                    optimizer_component_corr,
                                    mode=metric_mode,
                                    epoch=0,
    ...
    def train(self,train_loader,criterion,optimizer,optimizer_axis_calib,optimizer_metric2,optimizer_component_corr,epoch,mode=0,valid_loader=None,train_half=False,fix_order=False,decrease=False, no_edge_normalize=False):
        ...
        for step, (inputs, targets) in enumerate(train_loader):
            ...
            seen_train_batches.append((inputs,targets))
            ...
            if fix_order:
                ...
                if mode<6:
                    ...
                    inputs_search, targets_search = next(iter(seen_train_batches))
                    ...
                    if self.search_space.metric_num > 1:
                        if step < axis_calib_steps:
                            optimizer_axis_calib.zero_grad()
                            active_optimizer = optimizer_axis_calib
                            active_parameters = list(
                                _axis_calibration_weights(self.search_space)
                            )
                            active_stage = "axis_calib"
                        else:
                            optimizer_component_corr.zero_grad()
                            active_optimizer = optimizer_component_corr
                            active_parameters = list(
                                _component_corr_weights(self.search_space)
                            )
                            active_stage = "component_corr"
                    else:
                        optimizer_component_corr.zero_grad()
                        active_optimizer = optimizer_component_corr
                        active_parameters = list(
                    ...
                    if active_stage == "component_corr":
                        component_steps = max_train_steps - axis_calib_steps
                        if component_steps <= 1:
                            raise ValueError(
                                "component correction requires at least two steps"
                            )
                        local_step = step - axis_calib_steps
                        progress = float(local_step) / float(component_steps - 1)
                        base_lr = float(
                            os.environ.get("PROXYDIFF_COMPONENT_CORR_LR", "0.06")
                            or 0.06
                        )
                        scheduled_lr = 0.5 * base_lr * (
                            1.0 + math.cos(math.pi * progress)
                        )
                        for parameter_group in active_optimizer.param_groups:
                            parameter_group["lr"] = scheduled_lr
                        self.logger.info(
                            "proxydiff_component_lr_schedule: step={} local_step={} "
                            "progress={:.8f} lr={:.8f}".format(
                                step, local_step, progress, scheduled_lr
                            )
                        )

                    def task_objective(batch_inputs, batch_targets):
                        batch_logits = self.search_space.forward(
                            batch_inputs,
                            epoch=epoch,
                            to_print=False,
                            fix_logger=self.log_path,
                            no_edge_normalize=no_edge_normalize,
                        )
                        batch_loss = criterion(batch_logits, batch_targets)
                        return batch_logits, batch_loss

                    logits, primary_loss = task_objective(inputs, targets)
                    loss = primary_loss
                    loss.backward()
                    active_gradients = [
                        value.grad.detach().reshape(-1)
                        for value in active_parameters
                        if value.grad is not None
                    ]
                    if not active_gradients:
                        raise RuntimeError(
                            "no task gradient for {} parameters".format(active_stage)
                        )
                    active_gradient = torch.cat(active_gradients)
                    gradient_l2 = active_gradient.norm().clamp_min(
                        torch.finfo(active_gradient.dtype).eps
                    )
                    stage_aggregation_key = (
                        "PROXYDIFF_AXIS_GRADIENT_AGGREGATION"
                        if active_stage == "axis_calib"
                        else "PROXYDIFF_COMPONENT_GRADIENT_AGGREGATION"
                    )
                    gradient_aggregation = os.environ.get(
                        stage_aggregation_key,
                        os.environ.get(
                            "PROXYDIFF_GRADIENT_AGGREGATION",
                            (
                                "temporal_unit_direction"
                                if active_stage == "axis_calib"
                                else "stage_consensus_direction"
                            ),
                        ),
                    ).strip().lower()
                    apply_optimizer_step = True
                    if gradient_aggregation == "temporal_unit_direction":
                        unit_gradient = active_gradient / gradient_l2
                        decay = float(
                            os.environ.get("PROXYDIFF_DIRECTION_EMA_DECAY", "0.9")
                            or 0.9
                        )
                        previous_direction = stage_direction_ema.get(active_stage)
                        if previous_direction is None:
                            averaged_direction = unit_gradient
                        else:
                            averaged_direction = (
                                decay * previous_direction
                                + (1.0 - decay) * unit_gradient
                            )
                        stage_direction_ema[active_stage] = averaged_direction.detach()
                        direction_l2 = averaged_direction.norm().clamp_min(
                            torch.finfo(averaged_direction.dtype).eps
                        )
                        normalized_direction = averaged_direction / direction_l2
                        offset = 0
                        for value in active_parameters:
                            if value.grad is None:
                                continue
                            count = value.grad.numel()
                            value.grad.copy_(
                                normalized_direction[offset : offset + count].view_as(
                                    value.grad
                                )
                            )
                            offset += count
                        self.logger.info(
                            "proxydiff_gradient_aggregation: step={} stage={} "
                            "mode={} raw_l2={:.8f} direction_l2={:.8f} "
                            "decay={:.4f}".format(
                                step,
                                active_stage,
                                gradient_aggregation,
                                float(gradient_l2.item()),
                                float(direction_l2.item()),
                                decay,
                            )
                        )
                    elif gradient_aggregation in (
                        "stage_mean_unit_direction",
                        "stage_consensus_direction",
                    ):
                        previous_sum = stage_direction_sum.get(active_stage)
                        unit_gradient = _mean_unit_gradient([active_gradient])
                        direction_sum = (
                            unit_gradient
                            if previous_sum is None
                            else previous_sum + unit_gradient
                        )
                        direction_count = stage_direction_count.get(active_stage, 0) + 1
                        stage_direction_sum[active_stage] = direction_sum.detach()
                        stage_direction_count[active_stage] = direction_count
                        if active_stage == "axis_calib":
                            local_stage_step = step
                            stage_steps = axis_calib_steps
                            integrated_lr = float(
                                os.environ.get("PROXYDIFF_AXIS_CALIB_LR", "0.015")
                                or 0.015
                            ) * stage_steps
                        else:
                            local_stage_step = step - axis_calib_steps
                            stage_steps = max_train_steps - axis_calib_steps
                            base_lr = float(
                                os.environ.get("PROXYDIFF_COMPONENT_CORR_LR", "0.06")
                                or 0.06
                            )
                            integrated_lr = sum(
                                0.5 * base_lr * (
                                    1.0
                                    + math.cos(
                                        math.pi * float(index) / float(stage_steps - 1)
                                    )
                                )
                                for index in range(stage_steps)
                            )
                        apply_optimizer_step = local_stage_step == stage_steps - 1
                        if apply_optimizer_step:
                            mean_direction = direction_sum / float(direction_count)
                            direction_l2 = mean_direction.norm().clamp_min(
                                torch.finfo(mean_direction.dtype).eps
                            )
                            update_direction = (
                                mean_direction / direction_l2
                                if gradient_aggregation == "stage_mean_unit_direction"
                                else mean_direction
                            )
                            offset = 0
                            for value in active_parameters:
                                ...
                                offset += count
                            for parameter_group in active_optimizer.param_groups:
                                parameter_group["lr"] = integrated_lr
                            self.logger.info(
                                "proxydiff_gradient_aggregation: step={} stage={} "
                                "mode={} raw_l2={:.8f} mean_direction_l2={:.8f} "
                                "batch_count={} integrated_lr={:.8f} apply=1".format(
                                    step,
                                    active_stage,
                                    gradient_aggregation,
                                    float(gradient_l2.item()),
                            ...
                            self.logger.info(
                                "proxydiff_gradient_aggregation: step={} stage={} "
                                "mode={} raw_l2={:.8f} batch_count={} apply=0".format(
                                    step,
                                    active_stage,
                                    gradient_aggregation,
                                    float(gradient_l2.item()),
                    ...
                    elif gradient_aggregation == "raw_minibatch_gradient":
                        self.logger.info(
                            "proxydiff_gradient_aggregation: step={} stage={} "
                            "mode={} raw_l2={:.8f}".format(
                                step,
                                active_stage,
                                gradient_aggregation,
                                float(gradient_l2.item()),
                        ...
                        raise ValueError(
                            "unknown PROXYDIFF_GRADIENT_AGGREGATION={!r}".format(
                                gradient_aggregation
                            )
                        )
                    if apply_optimizer_step:
                    ...
                    active_weights = torch.cat(
                        [value.detach().reshape(-1) for value in active_parameters]
                    )
                    self.logger.info(
                        "proxydiff_train_step: step={} stage={} loss={:.8f} active_abs_max={:.8f} active_l2={:.8f}".format(
                            step,
                            active_stage,
                            float(loss.detach().item()),
                            float(active_weights.abs().max().item()),
                            float(active_weights.norm().item()),
                        )
                    )
    ...
    def get_score(self,epoch=0):
        if False:
            ...
            for cell in range(2):
                ...
                if self.model.metric_num != 0:
                    score[cell] += _component_correction(
                        _component_corr_weights(self.model)[cell]
                    )
                    score2 = score[cell]
                    score3 = F.leaky_relu(score[cell])
                else:
