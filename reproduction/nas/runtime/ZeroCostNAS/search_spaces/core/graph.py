...
from ZeroCostNAS.utils.utils import iter_flatten, AttrDict, get_config_from_args, set_seed, drop_path
from ZeroCostNAS.utils.logging import log_formats, log_first_n, setup_logger
from ZeroCostNAS.search_spaces.core.primitives import AbstractCombOp, AbstractPrimitive, Identity, Zero, ReLUConvBN
...
class Graph(torch.nn.Module, nx.DiGraph):
    ...
    **Interface to tabular benchmarks**
    ...
    def set_op_names(self, op_names):
    ...
    def forward(self, x, drop_prob=0, metric_para=None, mask=None, edge_mapping=None, skip_zero=None, to_print=False, fix_logger=None, no_edge_normalize=False, *args):
        """
        ...
        if to_print:
            new_logger = setup_logger(fix_logger + "/new_log_core.log")
            new_logger.setLevel(logging.INFO)
            # new_logger.info("no_edge_norm: "+str(no_edge_normalize))
            # new_logger.info("metric_para: "+str(metric_para))
            ...
            if "subgraph" in node:
                ...
                if len(node["input"].values()) == 1:
                    ...
                    if isinstance(comb_op, AbstractCombOp):
                        # if to_print:
                        #     new_logger.info("comb_op: " + str(comb_op))
                        ...
                        if comb_op==sum:
                            # if to_print:
                            #     new_logger.info("sum: " + str(len(input_tensors)))
                            if no_edge_normalize:
                            ...
                            x = comb_op(input_tensors)
                        # logger.info("after outer sum:" + str(x))
                        # if to_print:
                        #     new_logger.info("after outer sum:" + str(x))
            ...
            if (
                ...
                for neigbor_idx in self.neighbors(node_idx):
                    ...
                    if isinstance(edge_data.op, Graph):
                        ...
                        #     new_logger.info("list")
                        # try:
                        # if metric_para!=None:
                        # edge_output = sum(
                        #     [drop_path(edge_data.op[op_idx].forward(x, edge_data=edge_data), drop_prob) * metric_para[edge_idx][self.OP_NAMES.index(edge_data.op[op_idx].get_op_name)] for op_idx
                        #      in range(len(edge_data.op))])
                        edge_output = 0
                        metric_para_op_idx = None
                        # print(edge_data.op)
                        for op_idx in range(len(edge_data.op)):
                            # print(edge_data.op[op_idx].get_op_name)
                            if edge_data.op[op_idx].get_op_name in self.OP_NAMES:
                                # print('a')
                                metric_para_op_idx = self.OP_NAMES.index(edge_data.op[op_idx].get_op_name)
                                # if to_print:
                                #     new_logger.info(edge_data.op[op_idx].get_op_name)
                                # if skip_zero:
                                #     if metric_para_op_idx>skip_zero:
                                #         metric_para_op_idx = metric_para_op_idx - 1
                            else:
                                # print('b')
                                # print(self.OP_NAMES)
                                for name_idx in range(len(self.OP_NAMES)):
                                    # print(name_idx)
                                    # print(self.OP_NAMES[name_idx])
                                    if isinstance(self.OP_NAMES[name_idx],tuple):
                                        # print('c')
                                        if edge_data.op[op_idx].get_op_name in self.OP_NAMES[name_idx]:
                                            # print('d')
                                            metric_para_op_idx = name_idx
                                            break
                            # try:
                            # print(metric_para_op_idx)
                            # print(metric_para[edge_idx][metric_para_op_idx])
                            if edge_data.op[op_idx].get_op_name=="Zero":
                                edge_output += drop_path(edge_data.op[op_idx].forward(x, edge_data=edge_data), drop_prob)
                                # if to_print:
                                #     new_logger = setup_logger(fix_logger + "/new_log_core.log")
                                #     new_logger.setLevel(logging.INFO)
                                #     new_logger.info("a output: "+str(edge_output))
                            else:
                                if metric_para != None:
                                    if edge_mapping:
                                        edge_output += drop_path(edge_data.op[op_idx].forward(x, edge_data=edge_data), drop_prob) * metric_para[edge_mapping[edge_idx]][metric_para_op_idx] * mask[edge_mapping[edge_idx]][metric_para_op_idx]

                                    else:
                                        if to_print:
                                            new_logger.info("op input: " + str(x))
                                        edge_output += drop_path(edge_data.op[op_idx].forward(x, edge_data=edge_data), drop_prob) * metric_para[edge_idx][metric_para_op_idx] * mask[edge_idx][metric_para_op_idx]
                                        if to_print:
                                            new_logger.info("op output: "+str(edge_output))
                            # except Exception as e:
                            #     print(e)
                            #     print(edge_data.op[op_idx].get_op_name)
                            #     sys.exit()
                        # print("forward: " + str(edge_data.op))
                        # sys.exit()
                                else:
                                # if to_print:
                                #     for op_idx in range(len(edge_data.op)):
                                #         if isinstance(edge_data.op[op_idx], ReLUConvBN):
                                #             print(edge_data.op[op_idx].op[1].weight)
                                # print("forward: "+str(edge_data.op))
                                # sys.exit()
                                    if edge_mapping:
                                        # new_logger.info("before output: " + str(edge_output))
                                        edge_output += drop_path(edge_data.op[op_idx].forward(x, edge_data=edge_data), drop_prob) * mask[edge_mapping[edge_idx]][metric_para_op_idx]
                                        # if to_print:
                                        #     if mask[edge_mapping[edge_idx]][metric_para_op_idx]==1:
                                        #         new_logger.info("after output: " + str(edge_output))
                                    else:
                                        if to_print:
                                            new_logger.info("op input: " + str(x))
                                        edge_output += drop_path(edge_data.op[op_idx].forward(x, edge_data=edge_data), drop_prob) * mask[edge_idx][metric_para_op_idx]
                                        if to_print:
                                        #     new_logger = setup_logger(fix_logger + "/new_log_core.log")
                                        #     new_logger.setLevel(logging.INFO)
                                        #     new_logger.info("mask: " + str(mask[edge_mapping[edge_idx]][metric_para_op_idx]))
                                            new_logger.info("op output: "+str(edge_output))
                                        # edge_output = sum([drop_path(edge_data.op[op_idx].forward(x, edge_data=edge_data),drop_prob) for op_idx in range(len(edge_data.op))])

                            # if to_print:
                            #     new_logger = setup_logger(fix_logger + "/new_log_core.log")
                            #     new_logger.setLevel(logging.INFO)
                            #     new_logger.info("input,out: {}".format(x))
                            #     for op_idx in range(len(edge_data.op)):
                            #         if isinstance(edge_data.op[op_idx],ReLUConvBN):
                            #             new_logger.info("op_idx,weight: {},{}".format(op_idx, edge_data.op[op_idx].op[1].weight))
                            #         new_logger.info("op_idx,out: {},{}".format(op_idx,edge_data.op[op_idx].forward(x, edge_data=edge_data)))
                            #     sys.exit()
                        # edge_output = edge_output/ (len(edge_data.op))
                        # if not no_edge_normalize:
                        #     edge_output = edge_output/len(edge_data.op)
                        edge_idx += 1
                        # except Exception as e:
                        ...
                        # logger.info("single")
                        # logger.info("op:"+str(edge_data.op))
                        # for k,v in edge_data.op.named_parameters():
                        ...
                        if isinstance(edge_data.op, Graph):
                            ...
                            for primitive in edge_data.op.get_embedded_ops():
                                if isinstance(primitive, Graph):
                                    primitive.parse()
                        self.add_module(
                            "{}-edge({},{},0)".format(self.name, node_idx, neigbor_idx),
                            edge_data.op,
                        )
                    else:
                        idx_tmp = 0
                        for primitive in edge_data.op:
                            if isinstance(primitive, Graph):
                                primitive.parse()
                            elif primitive.get_embedded_ops():
                                for p in primitive:
                                    if isinstance(p, Graph):
                                        p.parse()
                            # if not isinstance(primitive, Zero()):
                            self.add_module(
                                "{}-edge({},{},{})".format(self.name, node_idx, neigbor_idx, idx_tmp),
                                primitive,
                            )
                            idx_tmp = idx_tmp+1
            self.is_parsed = True
