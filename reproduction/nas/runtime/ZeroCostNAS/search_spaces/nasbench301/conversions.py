...
def convert_naslib_to_genotype(naslib_object):
    ...
    for cell in cells:
        ...
        edge_op_dict = {}
        for i,j in cell.edges:
            if isinstance(cell.edges[i, j]["op"], list):
                multi_ops = True
                op_name = []
                for op in cell.edges[i, j]["op"]:
                    op_name.append(ops_to_genotype[op.get_op_name])
            else:
                multi_ops = False
                op_name = ops_to_genotype[cell.edges[i, j]["op"].get_op_name]
            edge_op_dict[(i,j)] = op_name
        if multi_ops:
            op_edge_list = [
                (edge_op_dict[(i, j)], i - 1, j-1)
...
def convert_genotype_to_naslib(genotype, naslib_object):
    ...
    for c, cell_type in enumerate(["normal", "reduce"]):
        ...
        if len(cell[0])>2:
            multi_op = True
            pre_head,pre_tail=-1,-1
            # edge: (op,head,tail), head start from 0
            for i, edge in enumerate(cell):
                head = edge[1]+1
                tail = edge[2]+1
                if multi_op:
                    if head==pre_head and tail==pre_tail:
                        edge_op_dict[cell_names[c]][(head, tail)].append(genotype_to_ops[edge[0]])
                    else:
    ...
    def add_genotype_op_index(edge):
        ...
        if (edge.head, edge.tail) in edge_op_dict[edge.data.cell_name]:
            # print("head,tail: {},{}".format(edge.head, edge.tail))
            # print("cell: "+str(edge.data.cell_name))
            if isinstance(edge_op_dict[edge.data.cell_name][(edge.head, edge.tail)], list):
                index = []
                for i, op in enumerate(edge.data.op):
                    # print(i,op.get_op_name)
                    if op.get_op_name in edge_op_dict[edge.data.cell_name][(edge.head, edge.tail)]:
                        index.append(i)
                    else:
                        for item in edge_op_dict[edge.data.cell_name][(edge.head, edge.tail)]:
                            if isinstance(item,tuple):
                                if op.get_op_name in item:
                                    index.append(i)
            else:
                for i, op in enumerate(edge.data.op):
                    if (
    ...
    def update_ops(edge):
        ...
        if len(edge.data.op)==8:
            primitives = edge.data.op
        else:
            primitives = edge.data.primitives

        if isinstance(edge.data.op_index, list):
            ops = [edge.data.op[idx] for idx in edge.data.op_index]
            if ops==[]:
                # primitives[1] is Zero
                ops.append(primitives[1])
        else:
            ops = primitives[edge.data.op_index]

        # edge.data.set("op", primitives[edge.data.op_index])
        edge.data.set("op", ops)
        edge.data.set("primitives", primitives)  # store for later use

        # print("update_idx: "+str(edge.data.op_index))
        # print("update: "+str(ops))
        # sys.exit()
    ...
    for i, cell_type in enumerate(["normal", "reduce"]):
        ...
        if len(cell[0])>2:
            ...
            for j in range(len(cell)):
                compact[i].append((cell[j][1], cell[j][2], OPS.index(cell[j][0])))
        else:
            multi_ops = False
            for j in range(len(cell)):
...
def convert_compact_to_genotype(compact):
    ...
    for i in range(2):
        ...
        if len(cell[0])>2:
            ...
            for j in range(len(cell)):
                try:
                    genotype[i].append((OPS[cell[j][2]], cell[j][0], cell[j][1]))
