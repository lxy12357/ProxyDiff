...
NAS_DIR="$(cd "$SCRIPT_DIR/../nas" && pwd)"
OP_SCORE_ROOT="${OP_SCORE_ROOT:-${NAS_DIR}/outputs/nb301_main/operation_scores}"
OUT_ROOT="${OUT_ROOT:-${NAS_DIR}/outputs/nb301_reported_component_ablation}"
...
FIXED_ARCH_FILE="${FIXED_ARCH_FILE:-${NAS_DIR}/assets/arch_dataset_20cell_c36.pt}"
FULL_SELECTION_DETAILS="${FULL_SELECTION_DETAILS:?set FULL_SELECTION_DETAILS to the full-pool details JSON}"
...
run_with_refinement_runtime "$REPORTED_DIR/build_nb301_component_ablation.py" \
  ...
  --selection-details "$FULL_SELECTION_DETAILS" \
  > "$OUT_ROOT/logs/build.log" 2>&1

run_with_refinement_runtime "$NAS_DIR/src/reevaluate_free_selected_arch.py" \
...
AXIS_CALIBRATION_LR=0.015 \
COMPONENT_CORRECTION_LR=0.060 \
COMPONENT_CORRECTION_SCALE=1.00 \
OPERATION_TOPK_COUNT=5 \
PROXYDIFF_FIXED_SAMPLER_SEED=9000 \
bash "$NAS_DIR/scripts/run_nb301_refinement.sh" \
